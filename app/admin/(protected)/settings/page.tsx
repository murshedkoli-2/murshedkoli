...
import { SECRET_MASK } from '@/lib/settings'
...
export default function SettingsManager() {
  ...
  const fetchLiveModels = async (explicitKey?: string) => {
    ...
    try {
      const res = await fetch('/api/ai/models', {
        ...
        body: JSON.stringify({ apiKey: explicitKey || (form.nvidiaNimKey === SECRET_MASK ? undefined : form.nvidiaNimKey) }),
      })
  ...
  useEffect(() => {
    ...
    // eslint-disable-next-line react-hooks/set-state-in-effect -- Starts an asynchronous API read; results arrive after I/O.
      load()
      fetch('/api/ai/models').then((res) => res.ok ? res.json() : null).then((data) => {
        if (Array.isArray(data?.models) && data.models.length) setAvailableModels(data.models)
      }).catch(() => {})
    }
  }, [ready, load])
  ...
  const save = async () => {
    ...
    try {
      ...
      for (const [key, value] of entries) {
        ...
        if (!response.ok) throw new Error('Setting save failed')
      }
      await load()
  ...
  const handleTestAI = async () => {
    ...
    try {
      const res = await fetch('/api/ai/test', {
        ...
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          apiKey: form.nvidiaNimKey === SECRET_MASK ? undefined : form.nvidiaNimKey,
